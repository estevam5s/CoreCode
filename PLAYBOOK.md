# CoreCode Local Agent Playbook

This playbook is a practical guide to run CoreCode with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, CoreCode starts directly.
...
Or let CoreCode recommend the best local model for your goal:
...
```powershell
Get-Content .\.corecode-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\corecode\corecode
bun run dev:profile
```
...
- Keep `.corecode-profile.json` local (already gitignored).
