...
const PROFILE_FILE_NAME = '.corecode-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('corecode');
  const launchCommand = configured.get('launchCommand', 'corecode');
  const terminalName = configured.get('terminalName', 'CoreCode');
...
async function launchCoreCode(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `CoreCode command not found: ${executable}. Install it with: npm install -g @gitlawb/corecode`,
      'Open Setup Guide',
      'Open Repository',
    );
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch CoreCode because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream CoreCode project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and CoreCode commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
function activate(context) {
  ...
  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'corecode.startInWorkspaceRoot',
    async () => {
      await launchCoreCode({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('corecode.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(CORECODE_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'corecode.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(CORECODE_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'corecode.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('corecode.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.corecode');
  });
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('corecode')) {
        refreshProvider();
      }
    }),
