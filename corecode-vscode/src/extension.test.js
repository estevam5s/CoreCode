...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'corecode',
    launchCommand: 'corecode --project-aware',
    terminalName: 'CoreCode',
    shimEnabled: false,
    workspaceFolder: '/workspace/corecode/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/corecode/very/long/path/example-project',
    launchCwdLabel: '/workspace/corecode/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/corecode/very/long/path/example-project/.corecode-profile.json',
    workspaceProfilePath: '/workspace/corecode/very/long/path/example-project/.corecode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the CoreCode wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/corecode\/very\/long\/path\/example-project"[^>]*>\/workspace\/corecode\/very\/long\/path\/example-project<\//,
  );
});
...
test('CoreCodeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { CoreCodeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/corecode/src/panels/control-center.js',
      workspacePath: '/workspace/corecode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/corecode/src/panels',
      projectAwareCwdLabel: '/workspace/corecode/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/corecode',
      workspaceRootCwdLabel: '/workspace/corecode',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/corecode',
      activeFilePath: '/workspace/corecode/src/panels/control-center.js',
      workspacePath: '/workspace/corecode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/corecode',
      projectAwareCwdLabel: '/workspace/corecode',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/corecode/src/panels',
      launchCwdLabel: '/workspace/corecode/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/corecode\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/corecode/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/corecode',
      launchCwdLabel: '/workspace/corecode',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/corecode/);
  assert.match(html, /Same workspace-root target as Launch CoreCode because the relative command resolves from the workspace root · \/workspace\/corecode/);
});
