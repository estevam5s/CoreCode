# CoreCode on Android (Termux)

A complete guide to running CoreCode on Android using Termux + proot Ubuntu.
...
CoreCode requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone CoreCode

```bash
git clone https://github.com/Gitlawb/corecode.git
cd corecode
npm install
npm link
```
...
### Step 6 — Build CoreCode

```bash
cd /data/data/com.termux/files/home/corecode
bun run build
```

You should see:
```
✓ Built corecode v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to CoreCode's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the CoreCode repo.
