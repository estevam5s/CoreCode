...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/corecode/workspace/.corecode-profile.json', 30),
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\corecode\\workspace\\.corecode-profile.json', 30),
    '...\\.corecode-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\corecode\\workspace\\.corecode-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.corecode-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/corecode',
    ...
    profileStatusHint: '/workspace/corecode/.corecode-profile.json',
    workspaceProfilePath: '/workspace/corecode/.corecode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
      ...
      rows: [
        {
          ...
          summary: 'corecode',
          detail: '/workspace/corecode · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/corecode/.corecode-profile.json',
          tone: 'neutral',
        },
      ],
    },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'corecode --project-aware',
    detail: 'Integrated terminal: CoreCode',
  });
